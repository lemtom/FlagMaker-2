...
public class FileHandler {
	...
	private static Overlay readOverlay(ArrayList<String> lines, int maximumX, int maximumY, String directory)
			throws Exception {
		...
		for (String line : lines) {
			...
			if (data[0].equals("overlay") || data[0].equals("type"))
				continue;
	...
	private static String getValue(ArrayList<String> data, String fieldName, String defaultValue) {
		return data.stream().filter(s -> s.toLowerCase().replace("\uFEFF", "").startsWith(fieldName.toLowerCase()))
				.findFirst().orElse(defaultValue).split("=")[1];
	}
}
