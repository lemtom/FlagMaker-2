...
public class OverlayBorder extends Overlay {
	public OverlayBorder(int maximumX, int maximumY) {
		super("border", new Attribute[] { new ColorAttribute("Color", Color.BLACK),
	...
	public OverlayBorder(Color color, double thickness, int maximumX, int maximumY) {
		super("border", new Attribute[] { new ColorAttribute("Color", color),
				new DoubleAttribute("Thickness", thickness, maximumX, true) }, maximumX, maximumY);
	}

	@Override
	protected Shape[] thumbnail() {
		return new Shape[] { new Path(new MoveTo(0, 5), new LineTo(30, 5), new LineTo(30, 25), new LineTo(0, 25),
				new LineTo(0, 5), new MoveTo(5, 10), new LineTo(5, 20), new LineTo(25, 20), new LineTo(25, 10),
				new LineTo(5, 10)) };
	}
	...
	public void draw(Pane canvas) {
		...
		Path path = new Path(new MoveTo(0, 0), new LineTo(canvas.getWidth(), 0),
				new LineTo(canvas.getWidth(), canvas.getHeight()), new LineTo(0, canvas.getHeight()), new LineTo(0, 0),
				new MoveTo(thickness, thickness), new LineTo(thickness, canvas.getHeight() - thickness),
				new LineTo(canvas.getWidth() - thickness, canvas.getHeight() - thickness),
				new LineTo(canvas.getWidth() - thickness, thickness), new LineTo(thickness, thickness));
	...
	public String exportSvg(int width, int height) {
		...
		return String.format(Locale.US,
				"<path d=\"M 0,0 %1$d,0 %1$d,%2$d 0,%2$d Z M %3$.3f,%3$.3f %4$.3f,%3$.3f %4$.3f,%5$.3f %3$.3f,%5$.3f Z\" %6$s fill-rule=\"evenodd\" />",
				width, height, thickness, width - thickness, height - thickness,
