...
public class OverlayDiamond extends OverlayShape {
	...
	protected Shape[] thumbnail() {
		return new Shape[] { new Path(new MoveTo(0, 15), new LineTo(15, 0), new LineTo(30, 15), new LineTo(15, 30),
				new LineTo(0, 15)) };
	}
	...
	public void draw(Pane canvas) {
		...
		double height = getDoubleAttribute("Height") == 0 ? width
		...
		Path path = new Path(new MoveTo(0, height / 2), new LineTo(width / 2, 0), new LineTo(width, height / 2),
				new LineTo(width / 2, height), new LineTo(0, height / 2));
	...
	public String exportSvg(int width, int height) {
		...
		double h = getDoubleAttribute("Height") == 0 ? w : height * (getDoubleAttribute("Height") / maximumY);

		double x = width * (getDoubleAttribute("X") / maximumX);
		double y = height * (getDoubleAttribute("Y") / maximumY);

		return String.format(Locale.US,
				"<polygon points=\"%1$.3f,%2$.3f %3$.3f,%4$.3f %1$.3f,%5$.3f %6$.3f,%4$.3f\" %7$s />", x, y - h / 2,
				x + w / 2, y, y + h / 2, x - w / 2, ColorExtensions.toSvgFillWithOpacity(getColorAttribute("Color")));
	}
}
