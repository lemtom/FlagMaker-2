...
public class OverlayHalfEllipse extends Overlay {
	...
	public OverlayHalfEllipse(Color color, double x, double y, double width, double height, double rotation,
	...
	protected Shape[] thumbnail() {
		return new Shape[] {
				new Path(new MoveTo(0, 20), new ArcTo(15, 15, 0, 30, 20, false, true), new LineTo(0, 20)) };
	}
	...
	public void draw(Pane canvas) {
		...
		Path path = new Path(new MoveTo(x1, y1), new ArcTo(radX, radY, angle * 180 / Math.PI, x2, y2, true, true),
				new LineTo(x1, y1));
	...
	public String exportSvg(int width, int height) {
		...
		return String.format(Locale.US, "<path d=\"M %.3f,%.3f A %.3f,%.3f %.3f 1,1 %.3f,%.3f z\" %s />", x1, y1, radX,
				radY, angle * 180 / Math.PI, x2, y2, ColorExtensions.toSvgFillWithOpacity(getColorAttribute("Color")));
	}
}
