...
public class OverlayLineVertical extends Overlay {
	public OverlayLineVertical(int maximumX, int maximumY) {
		super("line vertical", new Attribute[] { new ColorAttribute("Color", Color.BLACK),
				new DoubleAttribute("X", 1, maximumX, true), new DoubleAttribute("Thickness", 0.5, maximumX, false) },
				maximumX, maximumY);
	}
	...
	public void draw(Pane canvas) {
		Line line = new Line(canvas.getWidth() * getDoubleAttribute("X") / maximumX, 0,
				canvas.getWidth() * getDoubleAttribute("X") / maximumX, canvas.getHeight());
	...
	public String exportSvg(int width, int height) {
		return String.format(Locale.US,
				"<line x1=\"%.3f\" y1=\"0\" x2=\"%3f\" y2=\"%d\" stroke=\"#%s\" stroke-width=\"%.3f\" />",
				width * getDoubleAttribute("X") / maximumX, width * getDoubleAttribute("X") / maximumX, height,
