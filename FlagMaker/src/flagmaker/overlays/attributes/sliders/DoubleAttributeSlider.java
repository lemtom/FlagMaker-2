...
public class DoubleAttributeSlider extends NumericAttributeSlider {
	...
	public DoubleAttributeSlider(OverlayControl parent, String name, boolean isDiscrete, double value, int maximum,
			boolean useMaxX) {
		...
		chkDiscrete.selectedProperty()
				.addListener((ObservableValue<? extends Boolean> ov, Boolean oldval, Boolean newval) -> checkChanged());
		...
		slider.valueProperty().addListener((ObservableValue<? extends Number> ov, Number oldval, Number newval) -> {
			if (triggeredByUser && !oldval.equals(newval))
				sliderValueChanged();
			triggeredByUser = true;
		});
		ControlExtensions.hideControl(txtValue);
		txtValue.setOnKeyPressed(this::txtValueKeyDown);
	...
	private void sliderValueChanged() {
		lblValue.setText(
				isDiscrete ? String.format("%d", (int) slider.getValue()) : String.format("%.3f", slider.getValue()));
		valueChanged();
	}
	...
	private void setValueByFraction(double fraction) {
		...
		result = BigDecimal.valueOf(result).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
