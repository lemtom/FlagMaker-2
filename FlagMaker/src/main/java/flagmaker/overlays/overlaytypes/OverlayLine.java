...
public class OverlayLine extends Overlay {
	public OverlayLine(int maximumX, int maximumY) {
		super("line",
				new Attribute[] { new ColorAttribute("Color", Color.BLACK),
						...
						new DoubleAttribute("X2", 2, maximumX, true), new DoubleAttribute("Y2", 2, maximumY, false),
						new DoubleAttribute("Thickness", 0.5, maximumX, true) },
				maximumX, maximumY);
	}

	public OverlayLine(Color color, double x1, double y1, double x2, double y2, double thickness, int maximumX,
			int maximumY) {
		super("line",
	...
	public void draw(Pane canvas) {
		Line line = new Line(canvas.getWidth() * getDoubleAttribute("X1") / maximumX,
	...
	public String exportSvg(int width, int height) {
		return String.format(Locale.US,
				...
				width * getDoubleAttribute("X1") / maximumX, height * getDoubleAttribute("Y1") / maximumY,
				width * getDoubleAttribute("X2") / maximumX, height * getDoubleAttribute("Y2") / maximumY,
