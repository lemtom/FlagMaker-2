...
public class OverlayRays extends Overlay {
	public OverlayRays(int maximumX, int maximumY) {
		super("rays",
				new Attribute[] { new ColorAttribute("Color", Color.BLACK), new DoubleAttribute("X", 1, maximumX, true),
						new DoubleAttribute("Y", 1, maximumY, false), new IntegerAttribute("Count", 4, maximumX, true),
	...
	public OverlayRays(Color color, double x, double y, int count, double rotation, int maximumX, int maximumY) {
		super("rays", new Attribute[] { new ColorAttribute("Color", color), new DoubleAttribute("X", x, maximumX, true),
				new DoubleAttribute("Y", y, maximumY, false), new IntegerAttribute("Count", count, maximumX, true),
				new DoubleAttribute("Rotation", rotation, maximumX, true), }, maximumX, maximumY);
	}
	...
	public String exportSvg(int width, int height) {
		...
		for (String path : getPaths(width, height)) {
			sb.append(String.format(Locale.US, "<path d=\"%s\" %s />", path,
	...
	private String[] getPaths(double width, double height) {
		...
		for (int i = 0; i < count; i++) {
			Vector point1 = borderIntersection(centerX, centerY, angularInterval * 2 * i + rotationOffset, width,
					height);
			Vector point2 = borderIntersection(centerX, centerY, angularInterval * (2 * i + 1) + rotationOffset, width,
					height);
	...
	private static Vector borderIntersection(double centerX, double centerY, double angle, double width,
			double height) {
		...
		return !possiblePoints.isEmpty() ? possiblePoints.get(0) : new Vector(centerX, centerY);
	}
