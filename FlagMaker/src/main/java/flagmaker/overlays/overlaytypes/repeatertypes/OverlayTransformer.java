...
public class OverlayTransformer extends OverlayRepeater {
	public OverlayTransformer(int maximumX, int maximumY) {
		super("transformer", new Attribute[] { new DoubleAttribute("X", 1, maximumX, true),
				...
				new DoubleAttribute("Rotation", 0, maximumX, true), }, maximumX, maximumY);
	}

	@Override
	protected Shape[] thumbnail() {
		return new Shape[] { new Polygon(10, 10, 25, 10, 20, 20, 5, 20) };
	}
	...
	public String exportSvg(int width, int height) {
		...
		return String.format(Locale.US, "<g transform=\"matrix(%.3f,%.3f,%.3f,%.3f,%.3f,%.3f)\">%s</g>",
				matrix.getMxx(), matrix.getMxy(), matrix.getMyx(), matrix.getMyy(), matrix.getTx(), matrix.getTy(),
